...
public class HolidayHome extends Base {
	...
	public void enterCity() throws Exception {
		...
		driver.findElement(By.xpath(prop.getProperty("inputField"))).sendKeys(location[1]);// Passing city name to input
																							// field
		...
		Thread.sleep(1000);
		driver.findElement(By.xpath(prop.getProperty("inputField"))).sendKeys(Keys.ENTER);// Confirming value by
																							// entering it
	...
	public void inDate() throws Exception // Selecting check in date
	{
		...
		if ((location[2].equals(location[3])) || (location[2].equals(null) || location[3].equals(null)))// Checking
																										// input and
																										// output days
		{
		...
		if (location[2].equals(currentDate)) {
			...
			Thread.sleep(1000);
			driver.findElement(
					By.xpath("//div[@class=\"_2baLkbS8\"]/descendant::div[contains(@aria-label,'" + newDateIn + "')]"))
					.click(); // Selecting in-date in calendar

			Thread.sleep(1000);
			driver.findElement(
					By.xpath("//div[@class='_2baLkbS8']/descendant::div[contains(@aria-label,'" + newDateOut + "')]"))
					.click(); // Selecting out-date in calendar
		}
	...
	public void selectDateCheckIn() throws Exception // Selecting Check-In date
	{
		...
		List<WebElement> elements = driver.findElements(By.xpath(prop.getProperty("monthCatcher"))); // Fetching month
																										// details
		...
		if (lastYear == currentYear) // Check if current year is same as last year
		{
			...
			for (int i = 0; i < diff; i++) { // Clicking the right arrow to change months
			...
			try {
				for (int i = 0; i < elements.size(); i++) // Loop to traverse months
				{
					if (elements.get(i).getText().equals(myr)) // Matching input month with site data
					{
						...
						List<WebElement> days = driver.findElements(By.xpath(prop.getProperty("dayCatcher"))); // Fetching
																												// the
																												// days
	...
	public void selectDateCheckOut() throws Exception // Selecting Check-Out date
	{
		...
		for (int i = 0; i < diff; i++) {
			driver.findElement(By.xpath(prop.getProperty("arrowRight"))).click();// Clicking the right arrow to change
																					// months
		}
			...
			try {
				for (int i = 0; i < elements.size(); i++) // Loop to traverse months
				{
					if (elements.get(i).getText().equals(myr)) // Matching input month with site data
					{
						...
						List<WebElement> days1 = driver.findElements(By.xpath(prop.getProperty("dayCatcher"))); // Fetching
																												// the
																												// days
						Logger.log(Status.INFO, "Dates fetched successfully...");
	...
	public void travellerRating() throws Exception // Selecting Sort By option
	{
		...
		Logger.log(Status.PASS,
				MarkupHelper.createLabel("Traveller Rating Test Functionality Successfull", ExtentColor.BLUE));
	...
	public void elevatorOption() throws Exception { // Clicking show more option in Amenities
		...
		Logger.log(Status.PASS,
				MarkupHelper.createLabel("Elevator Testing Functionality Successfull", ExtentColor.BLUE));
	}
	...
	public void printOutput() throws Exception // Transferring data to excel sheet
	{
		...
		List<WebElement> titles = driver.findElements(By.xpath(prop.getProperty("listOfHotels"))); // Fetching list of
																									// hotels
		Logger.log(Status.INFO, "List of hotels fetched...");
		List<WebElement> price = driver.findElements(By.xpath(prop.getProperty("listOfPricePerNight"))); // Fetching
																											// list of
																											// price per
																											// night
		Logger.log(Status.INFO, "List of price per night fetched...");
		List<WebElement> total = driver.findElements(By.xpath(prop.getProperty("listOfTotalPrice"))); // Fetching list
																										// of total
																										// price for 5
																										// nights
		...
		Logger.log(Status.PASS,
				MarkupHelper.createLabel("Output data transfer Functionality Successfull", ExtentColor.BLUE));
	...
	public void NegThree(String str1) throws Exception {
		...
		driver.findElement(By.xpath(prop.getProperty("date2"))).click(); // Selecting a check-out date less than
																			// check-in date
		Thread.sleep(1000);
		if (driver.findElement(By.xpath(prop.getProperty("calenderDisp"))).isDisplayed())// Checking if check-out date
																							// is less than check-in
																							// date
